...
def create_training_curves():
    ...
    plt.tight_layout()
    plt.savefig(
        "../pictures/training_curves_comparison.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def create_reconstructions():
    ...
    plt.tight_layout()
    plt.savefig(
        "../pictures/reconstructions_comparison.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def create_pca_plots():
    ...
    for beta_name, beta_val in [("β1", 1.0), ("β2", 2.0), ("β4", 4.0)]:
        ...
        plt.tight_layout()
        plt.savefig(
            f"../pictures/pca_{beta_name.lower()}.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
...
def create_dsprites_samples():
    ...
    def create_shape(shape_type, position=(0.5, 0.5), scale=1.0):
        ...
        if shape_type == "square":
            ...
            y, x = np.ogrid[:64, :64]
            mask = (
                (x - center_x) ** 2 / (12 * scale) ** 2
                + (y - center_y) ** 2 / (8 * scale) ** 2
            ) <= 1
            img[mask] = 1
            ...
            y, x = np.ogrid[:64, :64]
            # Heart shape formula
            heart = ((x - center_x) ** 2 + (y - center_y) ** 2 - 8 * scale) ** 3 - (
                (x - center_x) ** 2
            ) * (y - center_y) ** 3 <= 0
            img[heart] = 1
...
if __name__ == "__main__":
    ...
    for beta, results in training_results.items():
        print(
            f"{beta}: Recon={results['recon']:.1f}, KL={results['kl']:.1f}, Total={results['total']:.1f}"
        )
