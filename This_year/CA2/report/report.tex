...
The dSprites dataset is a synthetic dataset of 2D shapes (squares, ellipses, and hearts) with five ground-truth latent factors: shape, scale, orientation, x-position, and y-position. Each image is 64x64 pixels in grayscale. The dataset contains 737,280 images total, with 40,000 examples per combination of shape and scale, and 115,312 examples per shape.
...
- Batch size: 64
- Latent dimension: 10
- Encoder architecture: Conv2d layers (1->32->64->128->256) with 4x4 kernels and stride 2
- Decoder architecture: ConvTranspose2d layers (256->128->64->32->1) with matching parameters
- Optimizer: Adam
- Loss function: Binary Cross Entropy + $\beta \cdot$ KL Divergence
...
The MIG values reported above were computed using the \texttt{compute\_mig} implementation in \path{This_year/CA2/codes/vae_implementation.ipynb} and saved to \path{This_year/CA2/results/mig_results.npy} (see repository for code and exact parameters).
...
\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{../pictures/pca_beta4.png}
...
\item \texttt{pca_beta1.png, pca_beta2.png, pca_beta4.png} (PCA visualizations for different $\beta$)
\end{itemize}
