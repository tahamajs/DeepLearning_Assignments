...
# # Problem overview
# 
# This notebook implements a Variational Autoencoder (VAE) for unsupervised representation learning and generation. The goals are:
# 
# - Learn a compact latent representation z for images using a probabilistic encoder q_phi(z|x) and decoder p_theta(x|z).
# - Train by maximizing a variational lower bound (ELBO) that balances reconstruction quality and latent regularization.
# - Demonstrate reconstruction, sampling, and basic latent traversals.
# 
# 
# # Mathematical foundation (concise)
# 
# Let x be an observed image and z a latent vector. A VAE optimizes the evidence lower bound (ELBO):
# 
# \[ \log p_\theta(x) \ge \mathbb{E}_{q_\phi(z|x)}[\log p_\theta(x|z)] - D_{KL}(q_\phi(z|x) \;||\; p(z)) \]
# 
# - Reconstruction term: encourages the decoder p_\theta(x|z) to explain x (often negative log-likelihood, e.g., MSE or Bernoulli/logits cross-entropy).
# - KL term: pushes the approximate posterior q_\phi(z|x) toward the prior p(z) (commonly N(0, I)).
# 
# Reparameterization trick (for Gaussian q_\phi(z|x) = N(\mu, \sigma^2)):
# 
...
# 
# - Encoder outputs mean \(\mu\) and log-variance \(\log\sigma^2\). Use small positive clamp on log-variance for numerical stability.
# - Decoder maps z back to image space. Choose final activation according to output distribution (sigmoid for [0,1] images, identity for scaled MSE).
# - Typical latent sizes: 16, 32, 64. Smaller sizes encourage stronger compression and disentanglement; larger sizes yield higher fidelity.
# 
# # Loss and training tips
# 
# - Total loss = ReconstructionLoss(x, x_hat) + \(\beta\) * KL(q||p). Using a weight \(\beta\) (Beta-VAE) helps control disentanglement.
# - Use Adam optimizer (lr 1e-3 or 3e-4). Train until reconstruction + KL converge.
# - Warm-up KL: linearly increase \(\beta\) from 0 to 1 during first N epochs to avoid posterior collapse.
# - Monitor: reconstruction loss, KL divergence, and ELBO (sum) separately.
# 
# # Evaluation & experiments
# 
# - Reconstructions: feed validation images through encoder->decoder and show input/ reconstruction side-by-side.
# - Sampling: sample z~N(0,I) and decode to visualize generations.
# - Latent interpolation: linear interpolate between two latent vectors z1 and z2 to study smoothness.
# - Quantitative metrics: reconstruction MSE/CE, (optionally) FID or IS for generative quality if dataset size permits.
# 
# # Reproducibility checklist
# 
# - Fix random seeds for numpy/torch and set deterministic cuDNN flags if exact reproducibility is required.
# - Save model checkpoints and training logs (loss curves).
# - Document latent size, learning rate, batch size, number of epochs, and any KL warm-up schedule used.
# 
# 
# # Quick suggestions to extend this notebook
# 
# - Add Beta-VAE experiments (vary \beta and report disentanglement vs. reconstruction quality).
# - Replace convolutional encoder/decoder with residual blocks or upsampling via sub-pixel convs for higher fidelity.
# - Add conditional VAE (include class label or conditioning vector) if labels are available.
# 
# 
# *Inserted: problem overview, concise theory, practical tips, and reproducibility notes.*
...
def evaluate_vae_reconstruction(model, test_loader, device):
    ...
    with torch.no_grad():
        for images, _ in test_loader:
            ...
            if len(reconstructions) < 10:  # Save first 10 for visualization
